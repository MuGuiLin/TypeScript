...
var LocalDB = /** @class */ (function () {
    ...
    LocalDB.prototype["delete"] = function (id, cb) {
        if (id) {
            ...
            console.log("\u7528\u6237".concat(id, "\u5DF2\u5220\u9664\u6210\u529F\uFF01"));
            cb && cb();
            return true;
        }
        else {
    ...
    LocalDB.prototype.update = function (obj, id, cb) {
        ...
        if (data) {
            this.DB.setItem(id, JSON.stringify(obj));
            console.log("\u7528\u6237".concat(id, "\u5DF2\u66F4\u65B0\u6210\u529F\uFF01"));
            cb && cb();
            return true;
        }
        else {
    ...
    if (data) {
        ...
        for (var key in data) {
            if ('string' == typeof data[key]) {
                ...
                dom += "<tr><td>".concat(key, "</td><td>").concat(obj.username, "</td><td>").concat(obj.password, "</td><td><button type=\"button\" onClick=\"Remove(").concat(key, ")\">\u5220 \u9664</button></td><td></td></tr>");
            }
        }
        $('#tbody').innerHTML = dom;
    }
